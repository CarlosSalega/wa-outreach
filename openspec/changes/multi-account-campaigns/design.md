...
- [ ] Should `AppConfig` be removed entirely post-migration or kept as a thin global-settings singleton?
- [ ] QR regeneration UX: auto-refresh or manual button per account? (proposal says button)
- [ ] Worker lifecycle: restart policy for disconnected accounts? Currently skip in scheduler — explicit restart via API.
